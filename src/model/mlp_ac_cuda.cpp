...
// Xavier初始化函数
void xavier_init(const torch::nn::LinearImpl *linear) {
  float gain = 1.0f;
...
MlpAC::MlpAC(unsigned int obs_dim, unsigned int n_actions,
    ...
    : AC_Base(obs_dim, n_actions), obs_dim(obs_dim), hidden_dim(hidden_dim),
      action_dim(n_actions), shared(nullptr), actor(nullptr), critic(nullptr),
      actor_head(nullptr), critic_head(nullptr) {
  // 创建网络层
